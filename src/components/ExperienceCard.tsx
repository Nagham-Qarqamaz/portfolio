...
function ExperienceCard({
    ...
    return (
        <div className={`relative ${className}`}>
            <FadeIn>
                <div
                    className={`hidden absolute top-[50%] w-[28rem] xl:w-[42rem] 2xl:w-[56rem] h-32 border-t-4 border-primary-500 border-dashed -z-10 ${
                        dashedLine == "onRight" &&
                        "md:block left-1/2 border-r-4 rounded-tr-[40px]"
                    } ${
                        dashedLine == "onLeft" &&
                        "md:block right-1/2 border-l-4 rounded-tl-[40px]"
                    }`}
                />
            </FadeIn>
            {side == "left" ? (
